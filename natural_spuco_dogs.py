...
def load_hyperparams(config_path: str = "hyperparams_spuco_dogs.json") -> Dict[str, Any]:
...
def fine_tune(classifier, dataloader, fname, method='Orig', save_model=True, seed=10, config=None, model_save_dir='../SCRATCH/CFE_models') -> float: # type: ignore
    
    # Use config values if provided
    epochs = config['training']['epochs'] if config else 60
    lr = config['optimizer']['lr'] if config else 1e-3
    momentum = config['optimizer']['momentum'] if config else 0.9
    weight_decay = config['optimizer']['weight_decay'] if config else 1e-4
    nesterov = config['optimizer']['nesterov'] if config else False
    grad_clip_norm = config['regularization']['grad_clip_norm'] if config else 1.0
...
class Main(LightningLite):
    def run(self,
        ...
        percentage: int,
        seed: int,
        config_path: str = "hyperparams_spuco_dogs.json"
    ) -> None:
        ...
        # Defining transformations using config values
        resize_size = config['data']['resize']
        ...
        # Loading the model using config values
        model = BinaryTrainedResNet(config['model']['architecture'], num_classes=config['model']['num_classes'])
        if fine_tune_model:
            model.train()
        else:
            print(f"Loading Pre-trained Model.")
            state_dict = torch.load(f"{model_save_dir}/SpuCO_dogs_trained_model_seed_{seed}.pt", map_location='cpu')
        classifier = ModelWithNormalization(model,
                                            mean=config['model']['normalization']['mean'], 
                                            std=config['model']['normalization']['std']
                    )
        if not fine_tune_model:
            classifier.load_state_dict(state_dict)
        ...
        # Fine Tuning the classifier
        classifier = self.setup(classifier)
        loss = fine_tune(classifier, train_dataloader, fname=fname, method='Orig', save_model=True, config=config, model_save_dir=model_save_dir) if fine_tune_model else None
        freeze(classifier)
        classifier.eval()
        ...
        # Defining the adversarial attacks and CounterFactual Explanations
        if norm == 'GDPR_CFE':
            gdpr_config = config['adversarial_attacks']['GDPR_CFE']
            atk = GDPR_CFE(
                    model=classifier,
            ...
            if train_vae:       
                ...
                vae_0_log_dict = train_vae_v1(
                    ...
                    lr_scheduler=torch.optim.lr_scheduler.ReduceLROnPlateau(optim_vae_0, min_lr=vae_train_config['lr_scheduler']['min_lr']),
                    save_model=f'{model_save_dir}/SpuCO_dogs_class_0_seed_{seed}.pt'
                )
                ...
                vae_1_log_dict = train_vae_v1(
                    ...
                    device=vae_1.device,
                    reconstruction_term_weight=vae_train_config['reconstruction_term_weight'],
                    lr_scheduler=torch.optim.lr_scheduler.ReduceLROnPlateau(optim_vae_1, min_lr=vae_train_config['lr_scheduler']['min_lr']),
                    save_model=f'{model_save_dir}/SpuCO_dogs_class_1_seed_{seed}.pt'
                )
            ...
            l2_config = config['adversarial_attacks']['L2']
            atk = BinaryPGDL2(classifier=classifier, 
            ...
            linf_config = config['adversarial_attacks']['Linf']
            atk = BinaryPGDLinf(classifier=classifier, 
        ...
        # Intialising the Adversarial model
        adv_model = BinaryTrainedResNet(resnet_type=config['model']['architecture'], num_classes=config['model']['num_classes'])
        adv_model.train()
        adv_classifier = ModelWithNormalization(adv_model,
                                            mean=config['model']['normalization']['mean'], 
                                            std=config['model']['normalization']['std']
        ...
        adv_classifier = self.setup(adv_classifier)
        adv_loss = fine_tune(adv_classifier, adv_dataloader, fname=fname, method=norm, save_model=True, config=config, model_save_dir=model_save_dir)
        freeze(adv_classifier)
        adv_classifier.eval()
...
if __name__ == '__main__':
    ...
    parser.add_argument('seed', type=int)
    parser.add_argument('devices', nargs='+', type=int)
    parser.add_argument('config', type=str, default='hyperparams_spuco_dogs.json', 
